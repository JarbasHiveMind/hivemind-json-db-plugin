"""End-to-end: a real JsonDB backend, driven through a live hivemind-core
...
pytest.importorskip("hivemind_json_database")
...
_JSON_PLUGIN = "hivemind-json-db-plugin"
...
class _HivescopeDBAdapter:
    ...
    def add_client(self, name, key, password=None, admin=False,
        ...
        # JsonDB persists on context-exit; hivescope calls add_client bare, so
        # commit now to survive the sync()/reload() the connect check performs.
        store = getattr(getattr(self._cdb, "db", None), "_db", None)
...
def _seed_legacy_blacklist(db, api_key: str, skills):
    ...
    blacklist lives in a top-level ``skill_blacklist`` key and the on-disk
    schema sentinel is rolled back to v1, exactly what an operator's DB
    looks like before the plugin upgrade runs.
    """
    for cid, rec in list(db._db.items()):
        if isinstance(rec, dict) and rec.get("api_key") == api_key:
            rec["skill_blacklist"] = skills
            db._db[cid] = rec
    db._db.store()
    db._write_schema_version(1)


def test_migrated_skill_blacklist_reaches_session():
    tmp = tempfile.mkdtemp()
    # JsonStorageXDG resolves its path lazily, so keep xdg patched for the
    # whole test, and commit the fresh (empty) store so reads don't raise
    # DatabaseNotCommitted before the first client is registered.
    patcher = mock.patch("hivemind_json_database.xdg_data_home", return_value=tmp)
    patcher.start()
